...
function vcontrol:init(args)
    ...
    self.channel = args.channel or "vol"
...
function vcontrol:update(status)
    ...
    local state  = "on" -- status:match("%[(o[nf]*)%]")
...
function vcontrol:get()
    self:mixercommand({ self.channel })
end

function vcontrol:up()
    self:mixercommand({ self.channel, "+", self.step })
end
...
function vwidget:update_widget(setting)
    self.widget:set_markup(
        "vol" .. self.widget_text[setting.state]:format(setting.volume))
end
